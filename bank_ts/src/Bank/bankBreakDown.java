...
public class bankBreakDown extends Frame implements ActionListener{

	
	// ArrayList
	private ArrayList<BankUserInfo> mem = new ArrayList<>();
	private ArrayList<bankBreakDownInfo> bkdi = new ArrayList<>();
	...
	Label userBreakDownName_la;
	
	TextArea userinfo_ta;
	...
	public void setFieldInit() {
		...
		userInfoLogo_la = new Label("거래 내역",Label.CENTER);
		userMessage_la = new Label("",Label.CENTER);
		userBreakDownName_la = new Label("[날짜]====[업체]====[입출금]====[잔액]",Label.CENTER);
		userinfo_ta = new TextArea();
	...
	public void setLayoutInit() {
		...
		center_p.setLayout(new GridLayout(2,1,2,2));
		center_p.add(logo_la);
		center_p.add(userInfoLogo_la);
		
		south_p.setLayout(new GridLayout(2,1));
		south_p.add(center_2_p);
	...
	public void getOpenLogin() {
		...
		loadCheck = breakdbLoard(bkdi);
		
		
		if(loginCheck == 1) {
			
			if(loadCheck == 1) {
				setopenDataList(bkdi,mem.get(memIndex).getBankNum());
			}		
	...
	public int breakdbLoard(ArrayList<bankBreakDownInfo> data) {
		
		// 카운터 및 오류 검사 버퍼.
		int buf = 0;
		
		String sql = "select * from bankbreakd";
		
		//String sql = "select * from BANKBREAKD";
		
		db_cl.setDBdriveInit();
		buf = db_cl.setBreakDouwnselect(sql,data);
	...
	public void setopenDataList(ArrayList<bankBreakDownInfo> data, String str) {
		
		String buf;
		
		for(int i = 0; i < data.size(); i++) {
		
			if(str.equals(data.get(i).getInteralBankNum())) {
				buf = "거래날짜 :";
				buf += data.get(i).getDateTime();
				buf += "\n";
				buf += "업체 :";
				buf += data.get(i).getUsingName();
				buf += "\n";
				buf += "금액 :";
				buf += data.get(i).getUsingCash();
				buf += "원";
				buf += "\n";
				buf += "입출금 :";
				buf += data.get(i).getTransaction();
				buf += "\t";
				buf += "잔액 :";
				buf += data.get(i).getTotalCash();
				buf += "원";
				buf += "\n";
				userinfo_ta.append(buf + "\n");
